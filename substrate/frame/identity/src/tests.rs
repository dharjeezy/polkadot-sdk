...
fn build_and_execute(test: impl FnOnce() -> ()) {
	new_test_ext().execute_with(|| {
		test();
		Identity::do_try_state().expect("All invariants must hold after a test");
	})
}
...
#[test]
fn try_state_detects_inconsistent_subs() {
	new_test_ext().execute_with(|| {
		let data = |x| Data::Raw(vec![x; 1].try_into().unwrap());
		let [one, _, _, _, ten, _, _, _] = accounts();
		...
		assert_ok!(Identity::add_sub(RuntimeOrigin::signed(ten.clone()), one.clone(), data(1)));
		assert_ok!(Identity::do_try_state());

		// The sub no longer points back to its super-identity.
		SuperOf::<Test>::remove(&one);
		assert!(Identity::do_try_state().is_err());
	});
}

#[test]
fn try_state_detects_super_without_identity() {
	new_test_ext().execute_with(|| {
		let data = |x| Data::Raw(vec![x; 1].try_into().unwrap());
		let [one, _, _, _, ten, _, _, _] = accounts();
		...
		// The identity is gone, but its subs are left behind.
		IdentityOf::<Test>::remove(&ten);
		assert!(Identity::do_try_state().is_err());
	});
}

#[test]
fn try_state_detects_judgement_of_unknown_registrar() {
	new_test_ext().execute_with(|| {
		let [_, _, three, _, ten, _, _, _] = accounts();
		...
		// A judgement given by a registrar that does not exist.
		IdentityOf::<Test>::mutate(&ten, |maybe_id| {
			let id = maybe_id.as_mut().unwrap();
			id.judgements.try_push((1, Judgement::Reasonable)).unwrap();
		});
		assert!(Identity::do_try_state().is_err());
	});
}
...
#[test]
fn try_state_detects_dangling_primary_username() {
	new_test_ext().execute_with(|| {
		let [_, _, _, _, ten, _, _, _] = accounts();
		let username = test_username_of(b"42".to_vec(), b"test".to_vec());

		// A primary username that was never registered in `UsernameInfoOf`.
		UsernameOf::<Test>::insert(&ten, username);
		assert!(Identity::do_try_state().is_err());
	});
}
